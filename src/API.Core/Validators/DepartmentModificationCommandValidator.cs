...
public class DepartmentModificationCommandValidator : AbstractValidator<DepartmentModificationCommand>
{
  public DepartmentModificationCommandValidator(
    ...
    RuleFor(x => x.ParentDepartmentId)
        .MustAsync(async (command, parentId, cancellationToken) =>
        {
          ...
          return await foundParentDepartment.Match(
            d => d.Match(
              department => department.TenantID == command.TenantID && department.ID != command.ID,
              () => false),
            () => true);
        })
        .WithMessage("Parent department must be in the same tenant and cannot be self-referential");
  }
}
